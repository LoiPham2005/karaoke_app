...
- [ ] Thay App ID test trong:
  ...
  - `ios/Runner/Info.plist` (`GADApplicationIdentifier` — line ~75)
...
- [ ] Setup Firebase Remote Config key `ad_config` (JSON match `AdConfig.toJson()` schema) — bắt buộc cho prod (`!FlavorConfig.isDev` → fetch).
  - Schema chính: `show_all_ads`, `enable_*`, `rules{...}`, `ad_units{inter, app_open, rewarded, native, banner}`.
  - JSON đầy đủ xem `.claude/skills/modules/SKILL.md` → "Production checklist (5 bước)".
- [ ] **Setup UMP (User Messaging Platform)** trên AdMob console — Privacy & messaging:
  - GDPR message (EU users)
  - Privacy options form (user có thể đổi consent từ Settings)
  - Bắt buộc — không có = AdMob disable account khi có EU traffic
- [ ] **Splash page** thêm `showAppOpenOnLaunch()` để AppOpen splash fire trên cold start:
  ```dart
  @override
  void initState() {
    super.initState();
  ...
  ```
- [ ] **Test device IDs**: lấy hash từ logcat sau lần load đầu (Android log: `Use RequestConfiguration.Builder().setTestDeviceIds(...)`) → thêm vào `AdManager._testDeviceIds` (auto rỗng ở prod)
- [ ] **Sửa `NSUserTrackingUsageDescription`** cho phù hợp app (ATT prompt iOS 14.5+)
- [ ] **iOS XIB cho native_full**: tạo `ios/Runner/NativeAdFullView.xib` qua Xcode Interface Builder (Android đã có `native_ad_full.xml` sẵn)
