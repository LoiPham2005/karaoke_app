...
| **Type-safe at compile time** | Codegen (riverpod, freezed, auto_route, envied, retrofit) |
...
## 7. Routing — auto_route typed

### Vì sao auto_route?
- Type-safe navigation qua codegen (`@RoutePage()` + `auto_route_generator`)
- Hỗ trợ nested router (`AutoTabsRouter` cho bottom nav) gọn hơn ShellRoute
- Path params + query params tự inject vào constructor
- Guards composable (`AutoRouteGuard`) — async support natively
- Deep linking + back stack đầy đủ
...
class AppRouter extends RootStackRouter {
  ...
  List<AutoRoute> get routes => [
    ...
    AutoRoute(page: MainRoute.page, path: '/', guards: [AuthGuard()], children: [
...
### Navigate
```dart
context.router.push(ProductDetailRoute(id: '123'));   // typed param
context.router.replaceAll([const MainRoute()]);       // clear stack
getIt<AppRouter>().push(const LoginRoute());          // không cần context
```
