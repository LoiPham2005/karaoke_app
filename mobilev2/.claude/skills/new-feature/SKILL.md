...
### Bước 4: Thêm route (auto_route)

**4.1 — Page annotation** (đã có nếu dùng Mason brick):
...
class ProductDetailPage extends HookConsumerWidget {
  ...
  final String id;  // ← auto map từ path param ':id'
  ...
}
```

**4.2 — Import page vào `lib/routes/config/app_router.dart`** ⚠️ (dễ quên — gây compile error vì `.gr.dart` là `part of`):

```dart
import 'package:flutter_base2/features/product/presentation/pages/product_list_page.dart';
import 'package:flutter_base2/features/product/presentation/pages/product_detail_page.dart';
```
  ...
  List<AutoRoute> get routes => [
    ...
    AutoRoute(
      ...
      children: [
        ...
        AutoRoute(page: ProductListRoute.page, path: 'products'),  // ← thêm vào shell
      ],
    ),
    // Detail top-level (không có bottom nav)
    AutoRoute(
...
// Từ service (không có context)
getIt<AppRouter>().push(const ProductListRoute());
```
...
| Page | `extends HookConsumerWidget`, `useAsyncValueChange(state)`, có `@RoutePage()` |
| Route | Import page vào `app_router.dart` + đăng ký `AutoRoute(page: XxxRoute.page, path: '...')` |
