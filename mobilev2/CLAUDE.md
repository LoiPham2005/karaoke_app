...
lib/
  ...
  routes/
    base/           — not_found_page
    config/         — app_router (@AutoRouterConfig), app_router.gr (generated), route_guards, safe_pop
```
...
| `.claude/skills/routing/` | auto_route typed, AutoTabsRouter, AutoRouteGuard, deep linking |
...
5. **LUÔN** chạy `make gen` sau khi sửa `@freezed`, `@RestApi`, `@riverpod`, `@injectable`, `@RoutePage`, `@Envied`
