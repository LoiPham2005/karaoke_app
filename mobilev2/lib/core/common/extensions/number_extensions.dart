...
/// Cached formatters cho case mặc định (vi_VN) — tránh tạo mới `NumberFormat`
/// mỗi lần gọi khi render list/card nhiều lần.
final _vndFmt = NumberFormat.currency(locale: 'vi_VN', symbol: '₫', decimalDigits: 0);
final _decimalVi = NumberFormat.decimalPattern('vi_VN');
...
extension NumberFormatX on num {
  ...
  String toCurrency({String locale = 'vi_VN', String symbol = '₫'}) {
    if (locale == 'vi_VN' && symbol == '₫') return _vndFmt.format(this);
  ...
  /// ```
  String toPercentage({int decimals = 1}) => '${(this * 100).toStringAsFixed(decimals)}%';
