...
class Validators {
  ...
  /// Email HOẶC số điện thoại VN — dùng cho ô đăng nhập (identifier).
  /// Có '@' → check format email, ngược lại → check format SĐT VN.
  static String? emailOrPhoneVN(String? value) {
    if (value == null || value.trim().isEmpty) {
      return 'Email/SĐT không được để trống';
    }
    final v = value.trim();
    return v.contains('@') ? email(v) : phoneVN(v);
  }
  ...
  /// Bọc validator thành OPTIONAL — bỏ qua khi field rỗng, chỉ check format
  /// khi user có nhập. Dùng cho field không bắt buộc (vd email/phone khi chỉ
  /// cần 1 trong 2).
  /// ```dart
  /// TextFormField(validator: Validators.optional(Validators.email))
  /// ```
