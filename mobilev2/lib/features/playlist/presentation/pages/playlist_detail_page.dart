...
class PlaylistDetailPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      ...
      body: switch (state) {
        AsyncData(:final value) => _PlaylistDetailContent(id: id, playlist: value),
...
class _PlaylistDetailContent extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final totalDuration = list.fold<int>(0, (sum, it) => sum + it.song.duration);
    ...
    Future<void> removeSong(String youtubeId) async {
      items.value =
          list.where((it) => it.song.youtubeId != youtubeId).toList();
      await ref.read(playlistsProvider.notifier).removeSong(id, youtubeId);
    }

    return CustomScrollView(
      slivers: [
        _appBar(context, list.length, totalDuration),
        SliverToBoxAdapter(
          child: Padding(
                ...
                Expanded(
                  child: SizedBox(
                    ...
                    child: ElevatedButton.icon(
                      onPressed: list.isEmpty || enqueuing.value ? null : playAll,
                  ...
                  child: Material(
                    ...
                    child: SongTile(
                      ...
                      onTap: () => context.router
                          .push(PlayerRoute(id: item.song.youtubeId)),
                      trailing: Row(
  ...
  Future<void> _saveOrder(
    ...
    try {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Không lưu được thứ tự')),
        );
      }
    }
  }

  Widget _appBar(BuildContext context, int count, int totalDuration) {
    return SliverAppBar(
                ...
                children: [
                  ...
                  if (playlist.description != null) ...[
                    ...
                    Text(
                      ...
                      style:
                          TextStyle(fontSize: 12.sp, color: context.textBody),
                    ),
                  ],
                  SizedBox(height: 4.r),
                  Text(
