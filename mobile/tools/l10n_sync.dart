  ...
  Future<SyncResult> _processFile(
    ...
    for (final key in sourceKeys) {
      if (!targetKeys.contains(key)) {
        ...
        _fixPlaceholders(key, sourceContent[key], targetContent, targetLocale);
      }
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (json.isNotEmpty && json[0] is List) {
          final translatedParts = (json[0] as List);
          var result = '';
...
class SyncResult {
  final int added, removed, updated;
  const SyncResult({
    required this.added,
