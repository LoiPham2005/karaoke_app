...
import 'package:flutter_base/shared/widgets/playlist_card.dart';
  ...
  Widget _buildPlaylists(BuildContext context) {
    return GridView.builder(
      ...
      itemBuilder: (_, i) {
        ...
        return PlaylistCard(playlist: pl, onTap: () => context.push('/playlist/${pl.id}'));
      },
    );
  }
  ...
  Widget _buildFavorites(BuildContext context) {
    return ListView.builder(
      ...
      itemBuilder: (_, i) => SongTile(
        ...
        index: i + 1,
        onTap: () => context.push('/song/${mockSongs[i % mockSongs.length].youtubeId}'),
      ),
    );
  }
    ...
    return ListView(
      ...
      children: [
        for (final (title, songs) in groups) ...[
          ...
          ...songs.map(
            (s) => SongTile(song: s, onTap: () => context.push('/song/${s.youtubeId}')),
          ),
        ],
      ],
    );
